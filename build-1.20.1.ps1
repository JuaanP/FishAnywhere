# Script para construir el mod para Minecraft 1.20.1
$version = "1.20.1"
...
# Configurar settings.gradle para incluir solo los loaders necesarios
$settingsGradle = @"
pluginManagement {
    repositories {
        gradlePluginPortal()
...
# Restaurar settings.gradle original
$originalSettings = Get-Content "settings.gradle.bak" -Raw
if ($originalSettings) {
    $originalSettings | Set-Content "settings.gradle"
} else {
    Write-Host "No se pudo restaurar settings.gradle original" -ForegroundColor Yellow
}
